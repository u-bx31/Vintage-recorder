...
export default function Home() {
  ...
  useEffect(() => {
    if (crackleEnabled && isPlaying) {
      if (!crackleAudioRef.current) {
        ...
        crackleAudioRef.current = new Audio('https://cdn.pixabay.com/download/audio/2022/03/15/audio_7d2f9b2b81.mp3?filename=vinyl-crackle-105151.mp3');
  ...
  const handleProgress = useCallback((currentTime: number, totalDuration: number) => {
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <header className="relative z-10 p-6 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-serif text-2xl font-bold text-amber-950 tracking-tight">Vintage Player</h1>
        </div>

        <div className="flex items-center gap-4">
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline" className="bg-amber-50/50 backdrop-blur-sm border-amber-900/20">
            ...
            <DialogContent className="sm:max-w-[600px]">
              ...
              <div className="grid md:grid-cols-2 gap-6 mt-4">
                <div>
                  <h4 className="text-sm font-medium mb-3 text-amber-950">Add New Track</h4>
                  <UploadPanel onAddTrack={handleAddTrack} />
                </div>
                <div>
                  <h4 className="text-sm font-medium mb-3 text-amber-950">Your Tracks</h4>
                  <TrackList 
                    tracks={tracks} 
          ...
          <Button 
            ...
            className={crackleEnabled ? "text-amber-900 bg-amber-200" : "text-amber-900/50 hover:text-amber-900"}
      ...
      <main className="flex-1 relative z-10 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-4xl mx-auto grid lg:grid-cols-1 gap-12 items-center">
          ...
          <div className="flex flex-col items-center justify-center space-y-12">
            <VintagePlayer 
              isPlaying={isPlaying} 
              coverArt={currentTrack?.type === 'youtube' ? `https://img.youtube.com/vi/${currentTrack.youtubeId}/hqdefault.jpg` : undefined}
            />
            ...
            <div className="text-center space-y-2 max-w-md w-full">
              ...
              <p className="text-amber-900/60 font-medium uppercase tracking-widest text-sm">
                {currentTrack ? (currentTrack.type === 'youtube' ? 'YouTube Audio' : 'Local Audio') : "Add tracks to begin"}
              </p>
            </div>
